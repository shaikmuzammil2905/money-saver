                    ...
                    {editingTheme.layout_data.map((blk, idx) => (
                      <div key={blk.id} className="bg-slate-950 border border-slate-800 rounded-2xl p-4 space-y-3 shadow-md">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
                          ...
                          {blk.type === 'button' && (
                            <div className="sm:col-span-3 bg-slate-900/90 p-2.5 rounded-xl border border-slate-700/80 space-y-2">
                              <label className="block text-[11px] font-bold text-amber-400">Target Action Link URL Box *</label>
                              <div className="flex flex-wrap sm:flex-nowrap gap-2 items-center">
                                <input
                                  type="text"
                                  value={blk.linkUrl || 'offers'}
                                  placeholder="e.g. offers, ott-plans, fiber, mobiles, /contact, https://..."
                                  onChange={(e) => handleUpdateBlockField(blk.id, 'linkUrl', e.target.value)}
                                  className="flex-1 bg-slate-950 border border-slate-700 rounded-xl p-2 text-white text-xs font-mono"
                                />
                                <select
                                  onChange={(e) => {
                                    if (e.target.value) handleUpdateBlockField(blk.id, 'linkUrl', e.target.value);
                                  }}
                                  className="w-32 bg-slate-800 border border-slate-700 rounded-xl p-2 text-xs font-bold text-slate-300"
                                >
                                ...
                                {blk.linkUrl && (
                                  <button
                                    type="button"
                                    onClick={() => {
                                      const url = blk.linkUrl;
                                      if (url.startsWith('http')) window.open(url, '_blank');
                                      else window.open(`/${url.replace(/^\//, '')}`, '_blank');
                                    }}
                                    className="px-2.5 py-2 bg-slate-800 hover:bg-slate-700 text-amber-300 rounded-xl text-xs font-bold shrink-0"
                                    title="Test Link"
                                  >
                                    🔗 Test
                                  </button>
                          ...
                          {/* Image Upload if type === 'image' */}
                          {blk.type === 'image' && (
                            <div className="sm:col-span-3 bg-slate-900/90 p-2.5 rounded-xl border border-slate-700/80 space-y-2">
                              <label className="block text-[11px] font-bold text-emerald-400">Image URL &amp; Cloudinary Uploader</label>
                              <div className="flex gap-2">
                                <input
                                  type="text"
                                  id={`img_input_${blk.id}`}
                                  value={blk.content || ''}
                                  placeholder="https://..."
                                  onChange={(e) => handleUpdateBlockField(blk.id, 'content', e.target.value)}
                                  className="flex-1 bg-slate-950 border border-slate-700 rounded-xl p-2 text-white text-xs font-mono"
                                />
                                <label className="px-3 py-2 bg-slate-800 hover:bg-slate-700 text-white rounded-xl text-xs font-bold cursor-pointer flex items-center gap-1 shrink-0">
                                  ...
                                  <input
                                    ...
                                    onChange={async (e) => {
                                      ...
                                      if (!file) return;
                                      try {
                                        const res = await uploadToCloudinary(file, 'theme_assets');
                                        handleUpdateBlockField(blk.id, 'content', res.url);
